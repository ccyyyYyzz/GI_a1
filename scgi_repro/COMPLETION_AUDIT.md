...
| M2 includes SCGI-network blind correction | Partial | `scgi_proxy` is dense-tested as an equal-frame blind smooth-gain proxy; `scgi_frozen` now loads a saved SCGI checkpoint and is dense-tested through the full prompt rho range in `results/phase_m2_scgi_frozen_dense_r1_highrho_merged` with 13,500 frozen-network rows. The direct cross-domain baseline underperforms (`srht_paired + pairwise` still wins 45/45 strict equal-frame cells), so a competitive fine-tuned network phase diagram remains open |
