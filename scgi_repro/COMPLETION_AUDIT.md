...
| M2 includes SCGI-network blind correction | Partial | `scgi_proxy` is dense-tested as an equal-frame blind smooth-gain proxy; `scgi_frozen` now loads a saved SCGI checkpoint and is smoke-tested in `results/phase_m2_scgi_frozen_smoke` with 210 frozen-network rows, but the direct cross-domain baseline underperforms and dense/fine-tuned network phase diagrams remain open |
