...
9. Mechanism-study Hadamard variants include natural, sequency, cake-cutting,
   and random row orders. Sequency is sorted by one-dimensional sign changes.
   Cake-cutting is implemented as a compact literature-inspired proxy that sorts
   square Hadamard rows by two-dimensional total variation, with sequency and
   natural index as deterministic tie-breakers; it is an ordering surrogate for
   mechanism tests, not a claim of bit-exact reproduction of any proprietary
   cake-cutting implementation.
10. Mechanism-study DCT uses full-contrast cosine rows with row-energy inverse
...
11. Current local verification uses the CUDA conda environment on the RTX 4060
    ...
    prompt thresholds.
12. `scgi.use_coord_channels` adds normalized row/column coordinates to the
...
13. `scgi.model_kind: gain_unet` makes the U-Net predict a positive gain map
...
14. `reference_kK` mechanism scans assume an inserted reference pattern with a
...
15. The nonideal digital-twin scans use normalized detector units rather than a
...
16. The `scgi_frozen` M2 smoke baseline loads a saved SCGI checkpoint and applies
...
17. Long-run CU accounting is estimated from wall-clock runtime and an explicit
