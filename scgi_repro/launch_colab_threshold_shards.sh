#!/usr/bin/env bash
# Launch the tall-design threshold full scan (Theorem A' validation) as two Colab shards.
# Usage (from WSL):  REF=<pushed-sha> bash launch_colab_threshold_shards.sh
set -euo pipefail
...
launch_job() {
  ...
  local run_id="$2"
  local shard_spec="$3"
  local out_dir="results/tall_design_threshold_full_${shard_spec/\//of}"
  ...
  nohup env HOME="$account_home" "$COLAB" --auth oauth2 run --session "$run_id" --gpu "$COLAB_GPU" --timeout "$TIMEOUT" \
    ...
    --command "python run_tall_design_threshold.py --output-dir ${out_dir} --shard ${shard_spec}" \
...
launch_job /var/tmp/codex-colab-accounts/pro1 "pro1_threshold_full_shard0of2_r1${RUN_ID_SUFFIX:-}" "0/2"
launch_job /var/tmp/codex-colab-accounts/pro2 "pro2_threshold_full_shard1of2_r1${RUN_ID_SUFFIX:-}" "1/2"
