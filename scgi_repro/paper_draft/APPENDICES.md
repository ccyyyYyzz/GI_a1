...
- **The manuscript's temporal-stationarity screen is controlled by acquisition-order structure.** The Brown–Forsythe/Levene screen of Sec. 7 probes temporal homogeneity of the acquired time series; it responds to whatever breaks the coarse-to-fine sequency-energy ordering of natural Hadamard rows — which either $P_{\mathrm{col}}$ **or** $P_{\mathrm{row}}$ accomplishes.
