...
*(iii) **Empirical support (numerical, not proof).** The $K=64$ tall-design threshold scan (Sec. 9, Fig. 8; 1300 cells: five drift dimensions $p\in\{3,5,9,17,33\}$, 20 seeds per cell) finds the local rank test transitioning from $0\%$ to $100\%$ pass **exactly** at $N=K+p-1$ for every $p$ — consistent with the uniform reading at the local-differential level, though a finite scan cannot prove it.*
