...
**Status of (B.5):** this is the *dominant-stratum hypothesis* — a genericity/dimension-count assertion (a single $(M,b)$ witness with full rank makes the relevant minor a not-identically-zero polynomial, hence nonzero generically). For the physical low-pass space it is *proved at generic-pair level* — for each fixed nonconstant $b$, with the exceptional design set allowed to depend on $b$ — in Sec. B.4–B.5 below; the uniform-over-$b$ version for a single generic $M$ is a labeled dimension-count argument (Prop. B.7, level (ii)). That is precisely why the low-pass lemmas matter.
...
**Proof.** By (B.6), the bad-object projection has dimension at most $\Delta=r+2K-1-N$ (Lemma B.2(2)). Generic-point identifiability holds when $\Delta<K-1$, i.e. $r+2K-1-N<K-1$, i.e. $N>K+r=K+p-1$; integrality gives (B.7). This direction inherits only the rank hypothesis (B.5) — rigorous once (B.5) is established (for low-pass $S$, at generic-pair level by Sec. B.5). For the converse, $N\le K+r$ gives $\Delta\ge K-1$, and under the dominant-projection assertion (Lemma B.2(3), genericity-flagged) the bad set fills $\mathbb P^{K-1}$, so a generic object has an alias. $\blacksquare$
...
**Proposition B.7 (low-pass transfer: a three-level hierarchy).** *Let $S=S_{\mathrm{LP}}$ as in (B.8), $p=2m+1<N$, and suppose the generic-point threshold $N\ge K+p$ holds. The transfer of the clean thresholds of Theorem A′(i)–(iii),*

$$
\text{local: } N\ge K+p-1,\qquad \text{generic: } N\ge K+p,\qquad \text{uniform: } N\ge 2K+p-1 ,
$$

*to the physical low-pass drift space holds at three distinct levels, which we state — and use — separately.*

*(i) **Generic-pair statement (proven).** For each **fixed** nonconstant $s\in S_{\mathrm{LP}}$, a generic $M$ — the exceptional design set may depend on $s$ — satisfies*
...
*i.e. the fixed pair $(M,s)$ behaves as the dense stratum of Sec. B.3. Given (B.11) at a generic pair, the sufficiency directions of Theorem A′(ii)–(iii) are rigorous for that pair; the necessity directions retain the dominant-projection flag of Lemma B.2(3).*

*(ii) **Uniform over the drift family for a single generic $M$ (dimension-count argument — not a theorem).** For fixed nonconstant $s$, let $c$ denote the codimension in $\mathbb R^{N\times K}$ of the bad-design set $X_s=\{M:\operatorname{rank}[M,-D_sM]<\min(N,2K)\}$ (a proper algebraic subset by level (i), so $c\ge1$). The nonconstant gain-ratio family is $(p-1)$-dimensional, so the union $\bigcup_{s}X_s$ is expected to have codimension $\ge c-(p-1)$, and a single generic $M$ avoids the entire union provided $c\ge p$. If the codimension of the rank drop matched the estimate for a* free *$N\times2K$ matrix family — corank one is determinantal of codimension $2K-N+1$ in the regime $K+p\le N<2K$ — then $c=2K-N+1\ge p$ gives the explicit sufficient window $N\le 2K-p+1$; and in the regime $N\ge2K$ the drop codimension is $N-2K+1$, so $c\ge p$ reads $N\ge 2K+p-1$ — recovering exactly the uniform threshold of Theorem A′(iii). The count is stated for the* constrained *family $[M,-D_sM]$ — the two blocks share the same $M$, and $D_s$ ranges over a structured exponential-trigonometric family — for which the free determinantal codimension is not automatic. We therefore label the whole of level (ii) a **dimension-count argument at heuristic level, not a theorem**. In particular, in the gap window $2K-p+1<N<2K+p-1$ even this count fails to cover the union, and only the generic-pair statement (i) is available there.*

*(iii) **Empirical support (numerical, not proof).** The tall-design threshold scans at $K=64$ (1300 cells: five drift dimensions $p\in\{3,5,9,17,33\}$, 20 seeds per cell) and $K=128$ (1950 cells: same five $p$, 30 seeds per cell) — Sec. 9, Fig. 8 — find the local rank test transitioning from $0\%$ to $100\%$ pass **exactly** at $N=K+p-1$ for every $p$ at both object sizes (3250 cells, no exceptions) — consistent with the uniform reading at the local-differential level, though a finite scan cannot prove it.*

**Proof of (i).** Fix a nonconstant $s\in S_{\mathrm{LP}}$. By Lemma B.5, $m_{\max}(s)\le p-1$. Apply Lemma B.6:
...
This arithmetic is exact and rigorous, and for fixed $s$ Lemma B.6's exceptional design set is a proper algebraic subset (depending on $D_s$), which is exactly the generic-pair claim. Levels (ii) and (iii) carry the statuses stated above and are **not** part of what is proven. $\blacksquare$
...
**What this does not claim.** The transfer is proven at the *generic-pair* level only: level (ii)'s uniform-over-drift statement is a labeled dimension-count argument — sufficient windows $N\le 2K-p+1$ and $N\ge 2K+p-1$ under the free determinantal codimension estimate, with the gap window $(2K-p+1,\,2K+p-1)$ covered at generic-pair level only — and level (iii)'s exact empirical $N=K+p-1$ rank transition supports, but does not prove, the uniform reading at the local level. Beyond that, the transfer says nothing about the regime $K<N<K+p$, where tallness is insufficient *even generically* and the statistical stationarity anchor of Sec. 5 (Theorem B) or a support/sparsity prior is required — the low-pass structure does not rescue algebraic identifiability below threshold, and at $p\approx N$ ($\rho_{\mathrm{bw}}\to1$) blind algebraic separation is impossible for any $N$. It does not claim well-conditioned inversion at or near $N=K+p$. It does not cover deterministic designs (the ordered-Hadamard obstruction of Theorem A is unaffected). And it certifies only the exact grid-Fourier below-Nyquist model; any other drift-class implementation must pass the $L_S$ check (B.12)–(B.13) before the clean thresholds are quoted.
...
**Status of the log expansion (flagged).** Step (iv) is a **controlled moment expansion, not a distribution-free theorem**: making the $O(\mathbb{E}|u_n|^3)$ remainder rigorous, and ensuring integrability of $\log B_n$ near $B_n=0$, requires a high-probability bound $P(|u_n|>\tfrac12)$ decaying fast enough — e.g. sub-exponential pattern entries plus $K_\infty$-driven concentration, or a strictly positive pattern floor $I\ge I_{\min}>0$. In the source derivation this step is a moment calculation; we use it only through the variance scale $\sigma_{\mathrm{abs}}^2=\sigma_{\mathrm{LR}}^2\asymp 1/K_{\mathrm{eff}}$ (per-frame, summable-absolute, and long-run variances all coincide for the i.i.d. carrier) entering rate constants, never in identifiability statements. Zero and near-zero buckets are excluded here; the calibrated soft-log $\psi_\alpha(c)=\log(c+\alpha)$ of Appendix D removes this restriction with a photon-dependent sensitivity factor.
...
- **(B2) Carrier noise.** $\{z_n\}$ is centered and strictly stationary, and *either* (B2a) $\alpha$-mixing with $\mathbb{E}|z_0|^{2+\eta}<\infty$ and $\sum_{h\ge1}\alpha(h)^{\eta/(2+\eta)}<\infty$, which by Davydov's covariance inequality [Davydov, *Theory Probab. Appl.* 13 (1968) 691] gives absolutely summable autocovariances — write $\gamma(h)=\operatorname{Cov}(z_0,z_h)$ and $\sigma_{\mathrm{abs}}^2:=\sum_{h\in\mathbb Z}|\gamma(h)|<\infty$ for the **summable-autocovariance constant**, the quantity entering every non-asymptotic variance bound below — and hence also a finite **signed long-run variance** $\sigma_{\mathrm{LR}}^2:=\gamma(0)+2\sum_{h\ge1}\gamma(h)\ (\le\sigma_{\mathrm{abs}}^2)$, which is the *asymptotic* variance of window means, $W\operatorname{Var}(W^{-1}\sum_{k\in W_n}z_k)\to\sigma_{\mathrm{LR}}^2$; the two constants play distinct roles and must not be conflated; *or* (B2b), for high-probability bounds, $\beta$-mixing with $\beta(k)\le\beta_0\exp[-(k/b)^\kappa]$ and $\|z_n\|_{\psi_1}\le M$.
...
**What this does not claim.** (F.2) requires the uncorrelatedness hypothesis $C_{\delta\xi}:=\operatorname{Cov}(\delta,\xi)=0$: without it, the same bias–variance decomposition produces exactly one additional term, $(2/S_2)\,\operatorname{tr}\!\big[L\,\mathrm{diag}(b)\,C_{\delta\xi}\,L^\top\big]$, which vanishes under the stated hypothesis but *not* automatically when the gain estimate $\hat a$ is computed from the same noisy record that carries $\xi$ (same-record dependence — the generic situation for a blind pipeline, and one the injected-residual protocol of Sec. 9 deliberately does not probe). Beyond that, (F.2) is a second-moment accounting identity for a *linear* reconstructor with the design held fixed; it does not bound nonlinear or data-adaptive $L$, and for random-pattern pipelines the constants below are obtained by additionally averaging over the pattern draw. It says nothing about how small $v$ can be made — that is the estimation-rate question of Sec. 6. Finally, the scalar collapse $v\,B_L$ *requires* uncorrelated residuals: for smooth (coherent) residual gain — the generic output of a windowed blind estimator — only the matrix form (F.4) is correct. This supersedes the earlier round-1 slogan "orthogonal inversion gives $v$," which holds only for independent coefficient-wise residuals under exact inversion.
...
**What Prop 3 does not claim.** The boundary compares two *specific* pipelines (pairwise-normalized Hadamard vs. blind random+DGI with the ideal correlator); it does not rank SRHT-paired pipelines, externally calibrated systems, or prior-regularized reconstructions, each of which shifts $\Delta_R$. It inherits every assumption of its ingredients: small mismatch (F.8), a measured pipeline $C_0$ and $D_H$, a stationary drift with a declared $r(\rho)$, and a $v_{\mathrm{blind}}$ from the window rate theorem whose constants are implementation-specific. It is a mean-square crossing, not a guarantee about any single realization; the single-crossing property itself is assumed, not derived; and at $\rho_{\mathrm{pair}}$ beyond the small-drift expansion only the defining infimum — not (F.12) — is meaningful.
