...
- **(B2) Carrier noise.** $\{z_n\}$ is centered and strictly stationary, and *either* (B2a) $\alpha$-mixing with $\mathbb{E}|z_0|^{2+\eta}<\infty$ and $\sum_{h\ge1}\alpha(h)^{\eta/(2+\eta)}<\infty$, which by Davydov's covariance inequality [26] gives absolutely summable autocovariances — write $\gamma(h)=\operatorname{Cov}(z_0,z_h)$ and $\sigma_{\mathrm{abs}}^2:=\sum_{h\in\mathbb Z}|\gamma(h)|<\infty$ for the **summable-autocovariance constant**, the quantity entering every non-asymptotic variance bound below — and hence also a finite **signed long-run variance** $\sigma_{\mathrm{LR}}^2:=\gamma(0)+2\sum_{h\ge1}\gamma(h)\ (\le\sigma_{\mathrm{abs}}^2)$, which is the *asymptotic* variance of window means, $W\operatorname{Var}(W^{-1}\sum_{k\in W_n}z_k)\to\sigma_{\mathrm{LR}}^2$; the two constants play distinct roles and must not be conflated; *or* (B2b), for high-probability bounds, $\beta$-mixing with $\beta(k)\le\beta_0\exp[-(k/b)^\kappa]$ and $\|z_n\|_{\psi_1}\le M$.
...
— Theorem C of the main text. In the photon-starved regime $\bar\lambda\ll1$ the variance term of (D.8) becomes $C/(W\bar\lambda)$, which is minimax-*order* sharp: the Poisson Fisher information for the log-intensity parameter is $I(\theta)=\lambda$, so $W$ counts carry information $\asymp W\bar\lambda$ and no estimator improves on the $1/(W\bar\lambda)$ order (van Trees [32]). Zeros only reduce Fisher information; they never make the estimator diverge.
...
a centered sum of $n=K/2$ draws *without replacement* from the finite population $\{w_1,\dots,w_K\}$ with mean $\mu_w=S_2/K$. Its exact variance is $n\sigma_w^2\frac{K-n}{K-1}\le\frac{K}{4}\sigma_w^2\le\frac14\sum_j w_j^2=\frac{S_2^2}{4K_4}$, where $\sigma_w^2=K^{-1}\sum_j(w_j-\mu_w)^2$; and each summand satisfies $0\le w_j\le\|T\|_\infty^2=S_2/K_\infty$. The Bernstein inequality for sampling without replacement (Serfling [25]; the Bernstein–Serfling form of Bardenet–Maillard [41]) then gives, for $t>0$,
...
- **The manuscript's temporal-stationarity screen is controlled by acquisition-order structure.** The Brown–Forsythe/Levene screen [44], [45] of Sec. 7 probes temporal homogeneity of the acquired time series; it responds to whatever breaks the coarse-to-fine sequency-energy ordering of natural Hadamard rows — which either $P_{\mathrm{col}}$ **or** $P_{\mathrm{row}}$ accomplishes.
