...
\]
The estimator is defined \emph{with the clamp}:
\begin{equation}
...
=V_n
\]
\emph{exactly}, and no mixing or stationarity assumption is required. If $\mathcal E_n=\{y_n\in M_n(\Theta)\}$, then
\begin{equation}
...
&+\frac{\bar\sigma_\alpha^2}{\underline\kappa_n^2\,W}\;+\;D_\Theta^2\,\mathbb{P}(\mathcal E_n^c),
\end{aligned}
\]
...
\paragraph{Low-photon corollary (window-average sensitivity).} For an unpadded equal-weight window, $d=0$, fixed $\alpha$, and $\max_{k\in W_n}\lambda_k(\ell_n)\le\varepsilon$, the per-frame expansions of the proof average into
\[
...
Le Cam's method~\cite{Tsybakov2009}. Let $v\perp\mathbf1$ be a bounded contrast supported
...
marginalization only decreases KL; in particular it does \emph{not}
route through Theorem~D's Gaussian bound (a Gaussian model is not a
submodel of the mixed-Poisson experiment).
...
The first term in (D.10) is the load-bearing obstruction. Ordinary
