...
\end{equation}
--- Theorem~C of the main text. In the photon-starved regime $\bar\lambda\ll1$ the variance term of (D.8) becomes $C/(W\bar\lambda)$, which is minimax-\emph{order} sharp: the Poisson Fisher information for the log-intensity parameter is $I(\theta)=\lambda$, so $W$ counts carry information $\asymp W\bar\lambda$ and no estimator improves on the $1/(W\bar\lambda)$ order (van Trees~\cite{GillLevit1995}). Zeros only reduce Fisher information; they never make the estimator diverge.
