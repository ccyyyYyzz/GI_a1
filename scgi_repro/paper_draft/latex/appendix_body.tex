...
\end{align*}
The $K_{\mathrm{eff}}K(\mu/\sigma)^2$ term is the familiar nonnegative-background penalty. For the ideal correlator, independence and unbiasedness give $\mathbb{E}_AC_0^{\mathrm{floor}}=C_0^{\mathrm{raw,moment}}$ and $\mathbb{E}_A\Lambda_{\mathrm{lev}}=1+C_0^{\mathrm{raw,moment}}$; \textbf{for a mean-subtracted or scale-aligned implementation these are not definition-level identities} --- each constant must be measured for the pipeline that is actually run. The clean-commit R15 contract output reproduces the historical grid's mean-subtracted \emph{raw} fixed-design floor near $10^3$: objectwise range $980.110$--$1114.529$, with the ten-object \emph{mean} agreeing with $K+\beta_4-2=1023.800$ to $0.8\%$. The historical \emph{scale-aligned} floor is near $6.2$--$7.2\times10^2$ and the nonnegative-background drift leverage near $10^6$; conflating any two is a factor-$1.5$ to factor-$10^3$ error.
...
\begin{statusblock}\textbf{Empirical status of Prop 3 (two estimators and a separate blind proxy).} The equal-frame grid ($N=2048$, $K=1024$, OU drift, 10 objects, five seeds, all constants measured, zero fitted parameters) is re-evaluated in raw MSE at \path{results/prop3_estimator_contract_r15/}. Its clean-commit manifest classifies it as the submission-authoritative R15 contract output; the prior raw-metric directory remains historical evidence rather than submission-authoritative output.
